...
extension BookMainViewController: FolioReaderDelegate, FolioReaderCenterDelegate, DictViewControllerDelegate {
    ...
    func loadLocation() -> Location? {
        APIService.shared.request(.putProgress(bookId: self.currentBookId!, progress: Progress(readPages: 0))).handle(ignoreError: false, type: Progress.self)  { offline, progress in }
        let pro = RealmService.shared.getEpubProgress(currentBookId!)
        let out = Location()
    ...
    func pageItemChanged(_ pageNumber: Int) {
        ...
        if lastPage != pageNumber || lastChapter != chapter {
            ...
            if interval > MinInterval && (lastChapter == chapter && pageNumber > lastPage || lastChapter != chapter && pageNumber == 1){
                ...
                payload.type = "epub"
                EventLogService.shared.send(payload)
                APIService.shared.request(.getProgress(bookId: currentBookId!))
                    .handle(ignoreError: false, type: Progress.self) { offline, progress in
                        if !offline {
                            var progress = progress!
                            progress.readPages = progress.readPages + 1
                            APIService.shared.request(.putProgress(bookId: self.currentBookId!, progress: progress)).handle(ignoreError: false, type: Progress.self)  { offline, progress in }
                        }
                }
            }
            lastPage = pageNumber
