...
function getSelectedSentence() {
    ...
    var sentenceIndex = $("p :in-viewport").get().flatMap(function(el, index) {
    ...
    var el = $("p :in-viewport").get().flatMap(function(el) {
                                  if (sel.containsNode(el, true)) {
                                  return [el];
                                  }
                                  return [];
    ...
    var index = sel.getRangeAt(0).toCharacterRange(el).start;
    return JSON.stringify({
        "index": index,
        "sentenceIndex": sentenceIndex,
        "word": sel.toString(),
          "sentence": rangy.innerText(el)
    });
}
...
document.addEventListener("selectionchange", function() {
    window.location = "selection-change://";
}, false);

                                                     
function getVisibleText() {
    return JSON.stringify($("p :in-viewport").toArray().map(function(li) {
                                                            return rangy.innerText(li);
                                                            }));
}
