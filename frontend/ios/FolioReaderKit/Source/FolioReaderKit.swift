...
public class Location: NSObject {
    @objc open var pageNumber: Int = 0
    @objc open var offsetX: Float = 0
    @objc open var offsetY: Float = 0
...
public class SelectedSentence: NSObject, Codable {
    @objc public var index: Int = 0
    @objc public var sentenceIndex: Int = 0
    @objc public var word: String = ""
    @objc public var sentence: String = ""
    override init() {
        super.init()
    }
}
...
@objc public protocol FolioReaderDelegate: class {
    ...
    @objc optional func folioReaderDidAppear(_ folioReader: FolioReader)
    ...
    @objc func selectionChanged(bookName: String, point: CGPoint, sentence: SelectedSentence?)
    
    // TODO manage state in folio reader
    @objc func ask(_ text: String?)
    
    @objc func saveLocation(_ location: Location?)
    
    @objc func loadLocation() -> Location?
}
...
extension FolioReader {
    ...
    open var savedPositionForCurrentBook: Location? {
        get {
            return delegate?.loadLocation()
        }
        set {
            delegate?.saveLocation(newValue)
        }
    }
}
...
extension FolioReader {
    ...
    @objc open func saveReaderState() {
        ...
        location.pageNumber = self.readerCenter?.currentPageNumber ?? 0
        location.offsetX = Float(webView.scrollView.contentOffset.x)
        location.offsetY = Float(webView.scrollView.contentOffset.y)
        location.progress = Float(self.readerCenter?.currentPageNumber ?? 0) / Float(self.readerCenter?.totalPages ?? 1)
        self.savedPositionForCurrentBook = location
    }
