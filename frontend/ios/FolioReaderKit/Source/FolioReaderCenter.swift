...
@objc public protocol FolioReaderCenterDelegate: class {
    ...
    @objc optional func currentText(_ sentences: [String])
    
    @objc optional func pageDidLoad()
}
...
open class FolioReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public var webViewPage: Int {
        if let webView = currentPage?.webView {
            ...
            return pageForOffset(contentOffset, pageHeight: pageSize)
        }
        return -1
    }
...
//        }
    ...
    func configureNavBar() {
        let navBackground = self.readerConfig.barColor
        let tintColor = readerConfig.barTintColor
    ...
    func configureNavBarButtons() {
        ...
        // Navbar buttons
        let audioIcon = UIImage(readerImageNamed: "icon-navbar-tts")?.ignoreSystemTint(color: self.readerConfig.barTintColor) //man-speech-icon
        let closeIcon = UIImage(readerImageNamed: "icon-navbar-close")?.ignoreSystemTint(color: self.readerConfig.barTintColor)
        let tocIcon = UIImage(readerImageNamed: "icon-navbar-toc")?.ignoreSystemTint(color: self.readerConfig.barTintColor)
        let fontIcon = UIImage(readerImageNamed: "icon-navbar-font")?.ignoreSystemTint(color: self.readerConfig.barTintColor)
        let space = 40 as CGFloat
        ...
        if self.book.hasAudio || self.readerConfig.enableTTS {
            let audio = UIBarButtonItem(image: audioIcon, style: .plain, target: self, action:#selector(presentPlayerMenu(_:)))
            audio.width = space
            rightBarIcons.append(audio)
        }
    ...
    func reloadData() {
        ...
        if self.readerConfig.loadSavedPositionForCurrentBook {
            guard let position = folioReader.savedPositionForCurrentBook, position.pageNumber > 0 else {
                self.currentPageNumber = 1
                return
            }
            self.changePageWith(page: position.pageNumber, animated: false, completion: { })
            self.currentPageNumber = position.pageNumber
        }
    }
    ...
    private func configure(readerPageCell cell: FolioReaderPage?, atIndexPath indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let jq2FilePath = Bundle.frameworkBundle().path(forResource: "isInViewport.min", ofType: "js")
        let rangyFilePath = Bundle.frameworkBundle().path(forResource: "rangy-core", ofType: "js")
        let rangy2FilePath = Bundle.frameworkBundle().path(forResource: "rangy-textrange", ofType: "js")
        ...
        let jsTag = "<script type=\"text/javascript\" src=\"\(jqFilePath!)\"></script><script type=\"text/javascript\" src=\"\(jq2FilePath!)\"></script><script type=\"text/javascript\" src=\"\(jsFilePath!)\"></script><script type=\"text/javascript\" src=\"\(rangyFilePath!)\"></script><script type=\"text/javascript\" src=\"\(rangy2FilePath!)\"></script>" +
    ...
    func updateCurrentText() {
        if let data = currentPage?.webView?.js("getVisibleText()")?.data(using: .utf8) {
            let sentences = try? JSONDecoder().decode([String].self, from: data)
            delegate?.currentText?(sentences ?? [])
        }
    }
    ...
    open func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        ...
        if scrollView is UICollectionView {
            ...
            collectionTimer = Timer(timeInterval: 2, target: self, selector: #selector(enableUserInteraction), userInfo: nil, repeats: false)
            RunLoop.current.add(collectionTimer!, forMode: RunLoop.Mode.common)
        }
        clearRecentlyScrolled()
        recentlyScrolled = true
        pointNow = scrollView.contentOffset
        if let currentPage = currentPage {
            currentPage.webView?.createMenu()
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if (isCollectionScrollView == false), let page = currentPage, let webView = page.webView {
            ...
            if page.lastOffset == nil {
                page.lastOffset = webView.scrollView.contentOffset
            }
            if contentOffset - page.lastOffset!.forDirection(withConfiguration: self.readerConfig) > pageSize {
                webView.scrollView.setContentOffset(page.lastOffset!, animated: false)
            } else {
                page.lastOffset = webView.scrollView.contentOffset
            }
...
extension FolioReaderCenter: FolioReaderPageDelegate {
    ...
    public func pageDidLoad(_ page: FolioReaderPage) {
        if self.readerConfig.loadSavedPositionForCurrentBook, let position = folioReader.savedPositionForCurrentBook {
            let pageNumber = position.pageNumber
            let offset = self.readerConfig.isDirection(CGFloat(position.offsetY), CGFloat(position.offsetX), CGFloat(position.offsetY))
