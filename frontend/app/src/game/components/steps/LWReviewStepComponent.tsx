...
const Initial: React.FC<{ store?: LWReviewStore }> = props => {
  ...
  return (
    <WordsCloud
      onDeselect={word => {
        selectedWords.current.delete(word);
      }}
      getVisibleWord={words => {
        visibleWords.current = words;
      }}
...
const LWReviewStepComponent: StepComponent = {
  storeGenerator: (step, review, rootStore) =>
    new LWReviewStore(step as LWReviewStep, review, rootStore),
  substeps: [Initial]
};
