  ...
  @action async download(book: Book) {
    if (isPlatform("electron")) {
      ...
      const file = fs.createWriteStream(
        path.join(this.booksDir(), book.id + ".book")
      );
      ...
      const lib = (supportedLibraries as any)[
        (parsed.protocol as string) || "http:"
      ];
      if (lib) {
        return new Promise<void>((resolve, reject) => {
          lib
            .get(book.downloadLink, (resp: any) => {
              resp.pipe(file);
              resolve();
            })
            .on("error", (e: any) => {
      ...
      return fileTransfer
        .download(
          book.downloadLink,
          File.dataDirectory + "books/" + book.id + ".book"
        )
        .then(() => {
